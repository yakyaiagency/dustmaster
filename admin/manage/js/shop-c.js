	...
	data.docs.forEach(doc => {
		...
		let item =
			`<tr data-id="${doc.id}">
					...
					<td>
							<a href="#" id="${doc.id}" class="edit js-edit-employee"> <button type="button" class="btn btn-success btn-sm btn-icon-text mr-3">
							แก้ไข
							<i class="typcn typcn-edit btn-icon-append"></i>                          
						  </button>
							</a>
							<a href="#" id="${doc.id}" class="delete js-delete-employee"> <button type="button" class="btn btn-danger btn-sm btn-icon-text">
							ลบรายการ
...
function checkemployee(){
    ...
    var flag = false;
    if(flag){
        document.getElementById("employee-nameError").style.display = "block";
    }else{
        document.getElementById("employee-nameError").style.display = "none";
    }
}
		...
		db.collection('employees').add({
			...
			createdAt : firebase.firestore.FieldValue.serverTimestamp()
				...
				Swal.fire({
					...
					setTimeout(function(){
						window.location.replace("/shop-c.html");
					}, 1000)
				});
